          ...
          <div className="flex flex-col gap-2">
              ...
              <div>
                ...
                {member?.joinDate && (
                  <div className="text-xs text-gray-500 pr-1 min-w-max">
                    Joined{" "}
                    {member?.joinDate && <Moment date={member?.joinDate?.toMillis()} fromNow />}
                  </div>
                )}
              </div>
            </div>
            <div className="flex gap-2 items-end">
              {member?.roles?.map((role, idx) => (
                <div
                  key={idx}
                  className="text-xs px-1 py-0.5 rounded-md font-medium bg-blue-100 text-blue-800 font-weight-600 font-space"
                >
                  {role}
                </div>
              ))}
...
export const Members = () => {
  ...
  useEffect(() => {
    const retrieveTokenGatedMembers = async () => {
      ...
      const tokenGatesSnap = await getDocs(
        query(collection(db, "gates"), where("roomId", "==", roomId)),
      );
